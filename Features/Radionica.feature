Feature: Radionica
  ...
  @Withdraw_Request
  Scenario Outline: Withdraw_Request
    ...
    And Assert page "Your request has been canceled" is loaded
...
#    And Assert process is complete
  ...
  Scenario Outline: Happy_path
    ...
    And Enter valid user email from excel "<rowindex>" columnname "email" in company review page
    And Enter valid user phone number from excel "<rowindex>" columnname "phone" in company review page
    And Click on button "Continue"
    And Assert page "Check your email" is loaded
